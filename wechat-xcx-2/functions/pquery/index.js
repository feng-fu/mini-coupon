...
exports.main = async (event, context) => {
  ...
  if (event.stype) {
    ...
    if (event.channel_type || event.channel_type === 0) {
      // 0-1.9包邮, 1-今日爆款, 2-品牌好货,3-相似商品推荐,4-猜你喜欢,5-实时热销,6-实时收益,7-今日畅销,8-高佣榜单，默认1
      args.channel_type = Number(event.channel_type);
    }
    // 官方活动
  }
  if (event.activity_tags && event.activity_tags.length) {
    // is_search = true;
    args.activity_tags = JSON.stringify(event.activity_tags);
  }
  // 生成推广
  if (event.generate) {
    is_search = false;
  ...
  // banner图，用来推广活动
  if (event.banner) {
    args.type = 'pdd.ddk.goods.search';
    console.log('activity_tags:', event.activity_tags)
    args.activity_tags = JSON.stringify(event.activity_tags);
  }
  // 详情
  if (event.detail) {
  ...
  try {
    ...
    if (is_search) {
      ...
      if (r.goods_search_response && r.goods_search_response.goods_list) {
        ...
        if (event.stype || event.banner) {
          const goods_search_response = r.goods_search_response || {};
          r.goods_basic_detail_response = {
            list: goods_search_response.goods_list || [],
            ...goods_search_response,
          };
          delete r.goods_search_response;
        }
        return {
          _status: 0,
          data: result.data,
